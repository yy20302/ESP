--[[
    luo_ye🍂 - Obsidian UI Script
    包含：ESP / Aimbot / Speed / Fly / Teleport / Noclip / Skins / Stalker
]]
...
--! Key System (已禁用 - 直接进入主 UI)
local KEY = "789113"
local KeyVerified = true  -- 默认已验证, 跳过卡密
...
--[[ KEY SYSTEM 原始代码已注释
    ...
    InputBox.Position = UDim2.new(0.1, 0, 0, 78)
    ...
    Status.Position = UDim2.new(0, 0, 0, 168)
    Status.Text = ""
...
CreateKeySystem()  -- 空函数, 立即返回


--! Embedded Obsidian Library (offline, no HttpGet required)
local Library = nil
do
    local SRC = [================[local cloneref = (cloneref or clonereference or function(instance: any)
    return instance
end)
...
function Library:GetIcon(IconName: string)
    if not FetchIcons or not Icons or not Icons.GetAsset then
        return
    end
...
return Library]================]
    ...
    if not fn then
        warn("[luo_ye] Obsidian 库加载失败: " .. tostring(loadErr))
    else
        local ok, lib = pcall(fn)
        if ok and type(lib) == "table" then
            Library = lib
        else
            warn("[luo_ye] Obsidian 库初始化失败: " .. tostring(lib))
        end
    end
end

if not Library then
    warn("[luo_ye] Obsidian 库加载失败, 使用 fallback 占位库 (UI 将无法显示)")
    Library = {
        Notify = function() end,
        Toggles = {}, Options = {}, Labels = {}, Buttons = {},
        CreateWindow = function() error("Obsidian not loaded") end,
    }
end


local success = true
local err = Library and "OK (embedded Obsidian)" or "FAIL"


-- ==================== 状态表 & 启动/停止函数 ====================
local ESP = {Enabled = false, TeamCheck = true, ShowBox = true, ShowName = true, ShowHealth = true, ShowDistance = true, ShowTracer = false, MaxDistance = 1500, BoxThickness = 1, TextSize = 14}
local Aimbot = {Enabled = false, ShowFOV = true, TeamCheck = true, aliveCheck = true, AimPart = "Head", aimModeType = "普通", FOVSize = 180, Smoothness = 5, AimSpeed = 8, Distance = 800}
local Speed = {Enabled = false, WalkSpeed = 32, JumpPower = 50}
local Fly = {Enabled = false, Speed = 80}
local Noclip = {Enabled = false}
local Stalker = {Enabled = false, Interval = 8}
local TPS = {Enabled = false, Distance = 10}
local Gyro = {Enabled = false, Speed = 360}
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer then
                plr.CharacterAdded:Connect(function(char)
                    task.wait(0.5)
                    pcall(function()
                        local hl = char:FindFirstChild("luo_ye_HL") or Instance.new("Highlight")
                        hl.Name = "luo_ye_HL"
                ...
                if plr.Character then
                    pcall(function()
                        local hl = plr.Character:FindFirstChild("luo_ye_HL") or Instance.new("Highlight")
                        hl.Name = "luo_ye_HL"
                        hl.FillTransparency = 0.5
...
local function StopESP()
    pcall(function()
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr.Character then
                local hl = plr.Character:FindFirstChild("luo_ye_HL")
...
local function StartAimbot()
    ...
    _aimConn = RunService.RenderStepped:Connect(function()
        pcall(function()
            ...
            local best, bestDist = nil, Aimbot.FOVSize
            for _, plr in ipairs(Players:GetPlayers()) do
                if plr ~= LocalPlayer and plr.Character and (not Aimbot.TeamCheck or not plr.Team or not LocalPlayer.Team or plr.Team ~= LocalPlayer.Team) then
                    local hum = plr.Character:FindFirstChildOfClass("Humanoid")
                    if (not Aimbot.aliveCheck or (hum and hum.Health > 0)) then
                        local part = plr.Character:FindFirstChild(Aimbot.AimPart or "Head")
                        if part then
                            local d = (part.Position - myRoot.Position).Magnitude
                            if d <= Aimbot.Distance then
                                local pos, onScreen = cam:WorldToViewportPoint(part.Position)
                                if onScreen then
                                    local center = Vector2.new(cam.ViewportSize.X / 2, cam.ViewportSize.Y / 2)
                                    local screenDist = (Vector2.new(pos.X, pos.Y) - center).Magnitude
                                    if screenDist < bestDist then bestDist = screenDist; best = part end
                                end
                            end
                        end
                    end
            ...
            if best then
                ...
                local sm = math.clamp(1 / math.max(Aimbot.Smoothness or 1, 1), 0.05, 1)
                cam.CFrame = cam.CFrame:Lerp(goal, sm)
            end
        end)
    end)
end
local function StopAimbot() if _aimConn then _aimConn:Disconnect(); _aimConn = nil end end

local _speedConn = nil
local function StartSpeed()
    if _speedConn then return end
    _speedConn = RunService.RenderStepped:Connect(function()
        pcall(function()
            ...
            hum.JumpPower = Speed.JumpPower or hum.JumpPower
        end)
    end)
end
...
local function StartFly()
    ...
    _flyConn = RunService.RenderStepped:Connect(function()
        pcall(function()
            if not _flyBV or not _flyBV.Parent then return end
            local cam = workspace.CurrentCamera
            ...
            _flyBV.Velocity = dir * (Fly.Speed or 80)
        end)
    end)
end
local function StopFly()
    if _flyConn then _flyConn:Disconnect(); _flyConn = nil end
    pcall(function() if _flyBV then _flyBV:Destroy(); _flyBV = nil end end)
    pcall(function() if _flyBG then _flyBG:Destroy(); _flyBG = nil end end)
end

local _nclipConn = nil
local function StartNoclip()
    if _nclipConn then return end
    _nclipConn = RunService.Stepped:Connect(function()
        pcall(function()
            local char = LocalPlayer.Character
            if not char then return end
...
local function StopNoclip() if _nclipConn then _nclipConn:Disconnect(); _nclipConn = nil end end

local _tpsConn = nil
local function StartTPS()
    if _tpsConn then return end
    _tpsConn = true
    pcall(function()
        LocalPlayer.CameraMode = Enum.CameraMode.Classic
        LocalPlayer.CameraMinZoomDistance = TPS.Distance
        LocalPlayer.CameraMaxZoomDistance = TPS.Distance
    end)
end
local function StopTPS() _tpsConn = nil end

local _gyroConn = nil
local function StartGyro()
    if _gyroConn then return end
    _gyroConn = RunService.RenderStepped:Connect(function(dt)
        pcall(function()
            local char = LocalPlayer.Character
            if not char then return end
            local hrp = char:FindFirstChild("HumanoidRootPart")
            if not hrp then return end
            hrp.CFrame = hrp.CFrame * CFrame.Angles(0, math.rad(Gyro.Speed or 360) * dt, 0)
        end)
    end)
end
local function StopGyro() if _gyroConn then _gyroConn:Disconnect(); _gyroConn = nil end end

local _stkThread = nil
local function StartStalker()
    if _stkThread then return end
    _stkThread = task.spawn(function()
        while Stalker.Enabled do
            task.wait(Stalker.Interval or 8)
            if not Stalker.Enabled then break end
            pcall(function()
                local pool = {}
                ...
                if #pool > 0 then
                    ...
                    local char = LocalPlayer.Character
                    if char and char:FindFirstChild("HumanoidRootPart") and pick.Character and pick.Character:FindFirstChild("HumanoidRootPart") then
                        char.HumanoidRootPart.CFrame = pick.Character.HumanoidRootPart.CFrame * CFrame.new(math.random(-3, 3), 0, math.random(-3, 3))
                    end
                end
            end)
        end
...
local function StopStalker() Stalker.Enabled = false end

function GetPlayerDropdownValues()
    local t = {}
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then table.insert(t, p.DisplayName) end
    end
    return t
end
function FindPlayerByDisplayName(name)
    for _, p in ipairs(Players:GetPlayers()) do
...
function Notify(msg, dur)
    pcall(function()
        Library:Notify({Title = "luo_ye", Content = tostring(msg), Duration = dur or 3})
    end)
end


if not Library or not Library.CreateWindow then
    warn("Cannot create window: Obsidian library not loaded")
    return
end
...
local Tabs = {
    ...
    Aimbot = Window:AddTab("自瞄"),
    Speed = Window:AddTab("加速"),
    Fly = Window:AddTab("飞行"),
    Teleport = Window:AddTab("传送"),
    Skin = Window:AddTab("美化"),
    Misc = Window:AddTab("杂项"),
    UISettings = Window:AddTab("UI 设置"),
}


-- ==================== ESP Tab ====================
local EspLeft = Tabs.Esp:AddLeftGroupbox("ESP 显示")
local EspRight = Tabs.Esp:AddRightGroupbox("ESP 设置")
...
EspLeft:AddToggle("ESPEnabled", {
    ...
    Callback = function(Value)
        ...
        Notify(Value and "ESP 已开启" or "ESP 已关闭")
    end,
})
...
EspLeft:AddToggle("ESPShowTracer", {
    Text = "显示射线",
...
EspRight:AddSlider("ESPBoxThickness", {
    Text = "方框线宽",
...
EspRight:AddSlider("ESPTextSize", {
    Text = "字体大小",
...
-- ==================== Aimbot Tab ====================
local AimLeft = Tabs.Aimbot:AddLeftGroupbox("自瞄设置")
local AimRight = Tabs.Aimbot:AddRightGroupbox("瞄准参数")
...
AimLeft:AddToggle("AimbotEnabled", {
    ...
    Callback = function(Value)
        ...
        Notify(Value and "自瞄已开启" or "自瞄已关闭")
    end,
})

AimLeft:AddToggle("AimbotShowFOV", {
    Text = "显示 FOV 圈",
    Default = Aimbot.ShowFOV,
    Callback = function(Value)
...
AimLeft:AddToggle("AimbotAliveCheck", {
    Text = "活体检测",
    Default = Aimbot.aliveCheck,
    Callback = function(Value)
...
AimLeft:AddDropdown("AimModeType", {
    Values = {"普通", "强锁"},
    Default = Aimbot.aimModeType,
    Text = "瞄准类型",
    Callback = function(Value)
        Aimbot.aimModeType = Value
    end,
})

AimLeft:AddKeyPicker("AimKeybind", {
    Text = "自瞄按键",
    Default = "Q",
    Mode = "Toggle",
    SyncToggleState = false,
...
AimRight:AddSlider("AimbotFOVSize", {
    Text = "FOV 范围",
    Default = Aimbot.FOVSize,
    Min = 50,
    Max = 500,
    Rounding = 0,
    Callback = function(Value)
...
AimRight:AddSlider("AimbotSmoothness", {
    Text = "平滑度",
...
AimRight:AddSlider("AimbotAimSpeed", {
    Text = "瞄准速度",
    Default = Aimbot.AimSpeed,
    Min = 1,
    Max = 20,
    Rounding = 0,
    Callback = function(Value)
        Aimbot.AimSpeed = Value
    end,
})

AimRight:AddSlider("AimbotDistance", {
    Text = "最大距离",
    Default = Aimbot.Distance,
    Min = 50,
    Max = 2000,
    Rounding = 0,
    Callback = function(Value)
...
-- ==================== Speed Tab ====================
local SpeedLeft = Tabs.Speed:AddLeftGroupbox("加速设置")
local SpeedRight = Tabs.Speed:AddRightGroupbox("按键")

SpeedLeft:AddToggle("SpeedEnabled", {
    Text = "加速开关",
    Default = Speed.Enabled,
    Callback = function(Value)
        Speed.Enabled = Value
        if Value then
            StartSpeed()
        ...
        Notify(Value and "加速已开启" or "加速已关闭")
    end,
})

SpeedLeft:AddSlider("SpeedWalkSpeed", {
    Text = "行走速度",
    Default = Speed.WalkSpeed,
    Min = 16,
    Max = 500,
    Rounding = 0,
    Callback = function(Value)
...
SpeedLeft:AddSlider("SpeedJumpPower", {
    Text = "跳跃力度",
    Default = Speed.JumpPower,
    Min = 50,
    Max = 500,
    Rounding = 0,
    Callback = function(Value)
        Speed.JumpPower = Value
    end,
})

SpeedRight:AddKeyPicker("SpeedKeybind", {
    Text = "加速按键",
    Default = "H",
    Mode = "Toggle",
    SyncToggleState = false,
...
-- ==================== Fly Tab ====================
local FlyLeft = Tabs.Fly:AddLeftGroupbox("飞行设置")
local FlyRight = Tabs.Fly:AddRightGroupbox("穿墙")

FlyLeft:AddToggle("FlyEnabled", {
    Text = "飞行开关",
    Default = Fly.Enabled,
    Callback = function(Value)
        Fly.Enabled = Value
        ...
        Notify(Value and "飞行已开启" or "飞行已关闭")
    end,
})

FlyLeft:AddSlider("FlySpeed", {
    Text = "飞行速度",
    Default = Fly.Speed,
    Min = 10,
    Max = 500,
...
FlyLeft:AddKeyPicker("FlyKeybind", {
    Text = "飞行按键",
    Default = "F",
    Mode = "Toggle",
    SyncToggleState = false,
...
FlyRight:AddToggle("NoclipEnabled", {
    Text = "穿墙 (Noclip)",
    Default = Noclip.Enabled,
    Callback = function(Value)
        ...
        Notify(Value and "穿墙已开启" or "穿墙已关闭")
    end,
})

FlyRight:AddKeyPicker("NoclipKeybind", {
    Text = "穿墙按键",
    Default = "N",
    Mode = "Toggle",
    SyncToggleState = false,
...
-- ==================== Teleport Tab ====================
local TeleportLeft = Tabs.Teleport:AddLeftGroupbox("玩家传送")
local TeleportRight = Tabs.Teleport:AddRightGroupbox("随机追踪")

local PlayerDropdownValue = nil

local PlayerDropdown = TeleportLeft:AddDropdown("TeleportPlayer", {
    Values = GetPlayerDropdownValues(),
    Default = 1,
    Text = "选择玩家",
    Callback = function(Value)
        PlayerDropdownValue = Value
    end,
})
...
TeleportLeft:AddButton({
    Text = "刷新玩家列表",
    Func = function()
        PlayerDropdown:Refresh(GetPlayerDropdownValues())
        Notify("玩家列表已刷新")
    end,
})
...
TeleportLeft:AddButton({
    Text = "传送到选中玩家",
    Func = function()
        local target = FindPlayerByDisplayName(PlayerDropdownValue or "")
        if not target then
            Notify("请先选择玩家")
            return
        end
        local ok = TeleportToPlayer(target)
        Notify(ok and ("已传送到 " .. target.DisplayName) or "传送失败")
    end,
})

TeleportRight:AddToggle("StalkerEnabled", {
    Text = "随机追踪",
    Default = Stalker.Enabled,
    Callback = function(Value)
        Stalker.Enabled = Value
        if Value then
            StartStalker()
        else
            StopStalker()
...
TeleportRight:AddSlider("StalkerInterval", {
    Text = "切换时间（秒）",
    Default = Stalker.Interval,
    Min = 3,
    Max = 60,
    Rounding = 0,
    Callback = function(Value)
        Stalker.Interval = Value
    end,
})


-- ==================== Skin Tab ====================
local SkinLeft = Tabs.Skin:AddLeftGroupbox("美化设置")
local SkinRight = Tabs.Skin:AddRightGroupbox("其他")

SkinLeft:AddLabel("Doors 楼层美化功能")
SkinLeft:AddLabel("根据游戏支持情况启用")
...
-- ==================== Misc Tab ====================
local MiscLeft = Tabs.Misc:AddLeftGroupbox("功能")
local MiscRight = Tabs.Misc:AddRightGroupbox("视角")

MiscLeft:AddToggle("TPSEnabled", {
    Text = "第三人称",
    Default = TPS.Enabled,
    Callback = function(Value)
        TPS.Enabled = Value
        ...
        Notify(Value and "第三人称已开启" or "第三人称已关闭")
    end,
})

MiscLeft:AddSlider("TPSDistance", {
    Text = "后肩距离",
    Default = TPS.Distance,
    Min = 1,
    Max = 30,
    Rounding = 0,
    Callback = function(Value)
...
MiscLeft:AddKeyPicker("TPSKeybind", {
    Text = "第三人称按键",
    Default = "T",
    Mode = "Toggle",
    SyncToggleState = false,
...
MiscRight:AddToggle("GyroEnabled", {
    Text = "大陀螺",
    Default = Gyro.Enabled,
    Callback = function(Value)
        Gyro.Enabled = Value
        if Value then
            StartGyro()
        ...
        Notify(Value and "大陀螺已开启" or "大陀螺已关闭")
    end,
})

MiscRight:AddSlider("GyroSpeed", {
    Text = "旋转速度（度/秒）",
    Default = Gyro.Speed,
    Min = 60,
    Max = 1080,
...
-- ==================== UI Settings Tab ====================
local UILayout = Tabs.UISettings:AddLeftGroupbox("菜单设置")

UILayout:AddToggle("ShowWatermark", {
    Text = "显示水印",
    Default = true,
    Callback = function(Value)
        local gui = game:GetService("CoreGui"):FindFirstChild("luo_ye_Watermark")
...
Notify("luo_ye🍂 Obsidian Edition 已加载")
